...
public abstract class SelectionModel {
    ...
    public Point lastPoint() {
        ...
        //  If there are no segments currently in the selection path, return the starting
        ...
        if (selection.isEmpty()) {
            ...
            int[] lastLineXs = lastLine.xs();
            int[] lastLineYs = lastLine.ys();
            return new Point(lastLineXs[lastLineXs.length - 1], lastLineYs[lastLineYs.length - 1]);
        }
    }
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        int closestIndex = -1;
        //Initialize the closest distance to distance of first point to given point
        int closestDistanceSq = (int) (Math.pow(selection.getFirst().start().x - p.x, 2)+Math.pow(selection.getFirst().start().y - p.y, 2));
        int countingIndex = 0;
        for (PolyLine lines: selection){
            //Checks closest point to all points in selection
            Point thisPoint = lines.start();
            int distanceSq = (int) (Math.pow(thisPoint.x - p.x, 2) + Math.pow(thisPoint.y - p.y, 2));

            if (distanceSq <= closestDistanceSq) {
                closestDistanceSq = distanceSq;
                closestIndex = countingIndex;
            }
            countingIndex++;
        }
        return closestIndex;
    ...
    protected void undoPoint() {
        if (selection.isEmpty()) {
            ...
            // Remove the last segment of the current selection path.  If the selection was
